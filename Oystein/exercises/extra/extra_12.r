...
nrep = 10
mvec = c(5,10,20)  #Subsample-size
#mvec = rep(1,3)
#deltavec= seq(0.5,1,length=3)
deltavec = rep(1,3)
l.rep = matrix(NA,nrow=3,ncol=nrep)
...
for(it in 1:100)
{
  ...
  l.rep[v,rep] = logl(y,d,mu.hat,tau2.hat,sig2.hat,phi.hat)
}
}
matplot(matrix(rep(mvec,nrep),ncol=nrep),l.rep,xlab="m",ylab="l",pch=1,col=1)
#matplot(matrix(rep(deltavec,10),ncol=10),Q.rep,xlab=expression(~delta),ylab="Q",
#        ylim=c(9925,11000),pch=1,col=1)
